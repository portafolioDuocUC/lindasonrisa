...
private DataSource ds;
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ProveedorADO sDAO = new ProveedorADO(ds.getConnection());
            sDAO.eliminar(rut);
            mensaje = "* Proveedor eliminado correctamente.";
        } catch (SQLException ex) {
            mensaje = "* Error. No se pudo eliminar el proveedor.";
            Logger.getLogger(EliminarProveedor.class.getName()).log(Level.SEVERE, null, ex);
        }
        request.setAttribute("msg", mensaje);

        request.getRequestDispatcher("ListarProveedor.do").forward(request, response);
    }
