...
public class ListarProductos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String mensaje = (request.getAttribute("msg") == null) ? "" : (String) request.getAttribute("msg");

        try {
            FamiliaProductoDAO fmDAO = new FamiliaProductoDAO(ds.getConnection());
            List<FamiliaProductoVO> listaFamilia = fmDAO.buscarTodos();
            session.setAttribute("listaFamilia", listaFamilia);
            
            TipoProductoDAO tpDAO = new TipoProductoDAO(ds.getConnection());
            List<TipoProductoVO> listaTipoProd = tpDAO.buscarTodos();
            session.setAttribute("listaTipoProd", listaTipoProd);
            
            ProductoDAO pDAO = new ProductoDAO(ds.getConnection());
            List<ProductoVO> listaProductos = pDAO.buscarTodos();
