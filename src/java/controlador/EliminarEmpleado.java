...
private DataSource ds;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            EmpleadoADO sDAO = new EmpleadoADO(ds.getConnection());
            sDAO.eliminar(run);
            mensaje = "* Empleado eliminado correctamente.";
        } catch (SQLException ex) {
            mensaje = "* Error. No se pudo eliminar el Empleado.";
            Logger.getLogger(EliminarEmpleado.class.getName()).log(Level.SEVERE, null, ex);
        }
        request.setAttribute("msg", mensaje);

        request.getRequestDispatcher("ListarEmpleado.do").forward(request, response);
    }
